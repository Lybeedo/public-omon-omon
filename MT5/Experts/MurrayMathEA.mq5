...
input ENUM_ORDER_TYPE_FILLING OrderFillMode   = ORDER_FILLING_FOK; // Fill policy
...
void OnDeinit(const int reason) {
    ...
    prefix = "MurrayMath_Fibo";
    ObjectsDeleteAll(0, prefix);
    prefix = "MurrayMath_VWAP";
    ObjectsDeleteAll(0, prefix);
...
bool PlaceOrder(ENUM_ORDER_TYPE type, double price, double sl, double tp, double lot, int magic, string cmt) {
    ...
    req.magic     = magic;
    req.comment   = cmt;
    req.type_filling = OrderFillMode;

    bool sent = OrderSend(req, res);
    if(!sent || res.retcode != TRADE_RETCODE_DONE) {
        PrintFormat("OrderSend FAILED! Retcode=%d Deal=%I64u", res.retcode, res.deal);
        return false;
    }

    PrintFormat("ORDER PLACED: %s %s %.2f @ %.5f SL=%.5f TP=%.5f | Ticket=#%I64u",
...
void ManageTrailing() {
    ...
    for(int i = PositionsTotal() - 1; i >= 0; i--) {
        ...
        } else if(posType == POSITION_TYPE_SELL && newSL < sl && sl == 0) {
            ModifySL(ticket, newSL);
        }
    }
}
